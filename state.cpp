...
double State::resolution = Grid_Res;
/**
 * Constructor
 */
State::State() {
...
State::State(double x, double y, double theta) {
  ...
  this->gx = x / resolution;
  this->gy = y / resolution;
...
std::vector<State> State::getNextStates() {
  ...
  double dist = 20.0;//30.0; // distance travelled within a unit time
       ...
       alpha += VEH_M_ALPHA) 
  {
    ...
    state.gx = state.x / resolution;
    state.gy = state.y / resolution;
