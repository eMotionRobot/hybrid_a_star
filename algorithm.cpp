...
int Algorithm::scale_map_x_ = MAPX;
int Algorithm::scale_map_y_ = MAPY;
int Algorithm::DX_ = DX;
int Algorithm::DY_ = DY;

// State previous[m_map.grid_x_][m_map.grid_y_][Theta];
/**
 * 2D Point type
 *
 */
...
Algorithm::Algorithm(Map map) {
  m_map = map;
  Algorithm::scale_map_x_ = m_map.map_x_;
  Algorithm::scale_map_y_ = m_map.map_y_;
  std::cout<<"m_map:"<<m_map.map_x_<<" "<<m_map.map_x_<<std::endl;
  // Initialize a geometric map and a grid map.
  obs_map = map.obs_map;
  grid_obs_map = new int *[Algorithm::DX_];

  // Intialize each grid value as 0.
  for (int i = 0; i < Algorithm::DX_; i++) {
    grid_obs_map[i] = new int[Algorithm::DY_];
    for (int j = 0; j < Algorithm::DY_; j++) {
      grid_obs_map[i][j] = 0;
    }
  }
  std::cout<<"Algorithm::DX:"<<Algorithm::DX_<<" "<<Algorithm::DY_<<std::endl;
  // Contruct occupied areas in grid_obs_map by obs_map.
  for (int i = 0; i < map.map_x_; i++) {
    for (int j = 0; j < map.map_y_; j++) {
      if (obs_map[i][j])
        grid_obs_map[i * Algorithm::DX_ / map.map_x_][j * Algorithm::DY_ / map.map_y_] = 1;
    }
  }
}
...
void Algorithm::astarPlanning() {
  ...
  std::cout<<"gx:"<<src.gx<<" gy:"<<src.gy<<std::endl;
  std::cout<<"grid_x_:"<<m_map.grid_x_<<" grid_y_:"<<m_map.grid_y_<<std::endl;
  src.dx = src.gx * Algorithm::DX_ / m_map.grid_x_;
  src.dy = src.gy * Algorithm::DY_ / m_map.grid_y_;
  std::cout<<"dx:"<<src.dx<<" dy:"<<src.dy<<std::endl;
  std::priority_queue<State, std::vector<State>, Compare2d> frontier;

  int visited[Algorithm::DX_][Algorithm::DY_];
  memset(visited, 0, sizeof(int) * Algorithm::DX_ * Algorithm::DY_);

  // Initialize cost array with the size of [Algorithm::DX_, Algorithm::DY_]
  double **cost = new double *[Algorithm::DX_];
  for (int i = 0; i < Algorithm::DX_; i++) {
    cost[i] = new double[Algorithm::DY_];
    for (int j = 0; j < Algorithm::DY_; j++) 
      cost[i][j] = 0;
  }
  ...
  while (!frontier.empty()) {
    ...
    for (int i = -1; i <= 1; i++)
    {
      for (int j = -1; j <= 1; j++)
      {
        ...
        if (x + i < 0 || x + i >= Algorithm::DX_ || y + j < 0 || y + j >= Algorithm::DY_) 
          continue;
        ...
        {
          //std::cout<<" -=>cost:"<<cost[x + i][y + j]<<"    "<<cost[x][y] + sqrt(i * i + j * j)<<std::endl;
        ...
        //std::cout<<" =>:"<<cost[x + i][y + j]<<std::endl;
      }
    }
  }
...
double Algorithm::holonomicWithObs(State src) {
  return Algorithm::shortest_2d[(int)src.x * Algorithm::DX_ / Algorithm::scale_map_x_][(int)src.y * Algorithm::DY_ / Algorithm::scale_map_y_];
}
 ...
 *
 */
std::vector<PathPoint>  Algorithm::hybridAstarPlanning()
{
  std::cout<<"Start astarPlanning"<<std::endl;    
  // Run the Dijkstra's Search Algorithm on the 2d grid map.
  // FutureWork: Try A star
  astarPlanning();
  std::cout<<"Get the Dijkstra's cost!"<<std::endl;   

  m_map.initCollisionChecker();
  std::cout<<"Finish initCollisionChecker"<<std::endl;  

  m_map.findNearObs();
  std::cout<<"Finish findNearObs"<<std::endl;  
  // -------------------------------------------------------
  ...
  int visited[m_map.grid_x_][m_map.grid_y_][Theta];
  memset(visited, 0, sizeof(int) * m_map.grid_x_ * m_map.grid_y_ * Theta);
  std::vector<PathPoint> path;
  previous_ = new State[m_map.grid_x_*m_map.grid_y_*Theta];
  while (pq.size() > 0) {
    State current = pq.top();
    pq.pop();
    //std::cout<<"cur:"<<current.gx<<","<<current.gy<<","<<current.gtheta<<" | "<<current.x<<","<<current.y<<","<<current.theta<<std::endl;
        ...
        abs(current.gtheta - goal.gtheta) <= 5) 
    {
      ...
      while (current.x != initial.x || current.y != initial.y ||
        ...
        dummy = previous_[current.gx + current.gy*m_map.grid_x_+current.gtheta*m_map.grid_x_*m_map.grid_y_];
        dummy.change =
    ...
    for (int i = 0; i < next.size(); i++) {
      //std::cout<<"  ->next:"<<next[i].gx<<","<<next[i].gy<<","<<next[i].gtheta<<" | "<<next[i].x<<","<<next[i].y<<","<<next[i].theta<<std::endl;
      ...
      if (!m_map.checkCollision(next[i])) {
        if (!visited[next[i].gx][next[i].gy][next[i].gtheta]) {
          ...
          //std::cout<<"    =>push:"<<next[i].cost3d+fmax(Algorithm::holonomicWithObs(next[i]), Algorithm::nonHolonomicWithoutObs(next[i]));
          pq.push(next[i]);

          // Construct edges (links). 
          previous_[next[i].gx + next[i].gy*m_map.grid_x_+next[i].gtheta*m_map.grid_x_*m_map.grid_y_] = current;               
        }
      }
    }
  }
