...
Map::Map(std::string map_file) {
  ...
  obs_map = new int *[map_x_];
  for (int i = 0; i < map_x_; i++) {
    obs_map[i] = new int[map_y_];
    for (int j = 0; j < map_y_; j++) {
      obs_map[i][j] = 0;
    }
  }
...
bool Map::initMap(std::vector<std::vector<int>>& maze_map)
{
  // Initilize the geometric map.
  obs_map = new int *[map_x_];
  for (int i = 0; i < map_x_; i++) {
...
void Map::initCollisionChecker() {
  acc_obs_map = new int *[map_x_];
  for (int i = 0; i < map_x_; i++) {
    acc_obs_map[i] = new int[map_y_];
    for (int j = 0; j < map_y_; j++) acc_obs_map[i][j] = obs_map[i][j];
  }

  for (int i = 0; i < map_x_; i++)
    for (int j = 1; j < map_y_; j++)
      acc_obs_map[i][j] = acc_obs_map[i][j - 1] + acc_obs_map[i][j];

  for (int j = 0; j < map_y_; j++)
    for (int i = 1; i < map_x_; i++)
...
bool Map::checkCollision(State pos) {
  ...
  if (pos.x >= map_x_ || pos.x < 0 || pos.y >= map_y_ || pos.y < 0 ||
      pos.theta >= Theta || pos.theta < 0)
    return true;

  if (pos.gx >= grid_x_ || pos.gx < 0 || pos.gy >= grid_y_ || pos.gy < 0 ||
  ...
  if (max_x >= map_x_ || min_x < 0 || max_y >= map_y_ || min_y < 0) return true;
...
bool Map::isBoundaryObstacle(int i, int j) {
  for (int k = i - 1; k <= i + 1; k++) {
    for (int l = j - 1; l <= j + 1; l++) {
      if (!(k >= 0 && k < map_x_ && l >= 0 && l < map_y_)) continue;
      if (obs_map[k][l] == 0) {
        return true;
      }
    }
...
void Map::findNearObs() {
  ...
  nearest_obstacle = new int *[map_x_];

  for (int i = 0; i < map_x_; i++) {
    nearest_obstacle[i] = new int[map_y_];
    for (int j = 0; j < map_y_; j++) {
